...
[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE) [![Upstream: Sveltia CMS](https://img.shields.io/badge/Forked%20from-Sveltia%20CMS-ff3e00)](https://github.com/sveltia/sveltia-cms)
...
### 8. 🔗 Internal Site Search in Link Inserter ("Insert Link")

- **Instant Internal Article Search**: The editor's link insertion dialog (`Insert Link` / `Ctrl+K`) features a built-in search bar to quickly search across all existing entries/articles of the site.
- **Smart URL Suggestion from Selected Text**: If the selected text in the editor is not an HTTP URL (e.g. keywords, article title, or subject), it automatically populates the search query and proposes the matching internal link (`/:year/:month/:day/:slug/` or configured `preview_path`) directly in the URL field.
- **Interactive Match Picker**: Displays search results with title, collection name, date, and internal URL. Clicking or double-clicking any result immediately sets the URL field (and the link text if not already defined), enabling friction-free cross-referencing between articles.
...
This project is open source and licensed under the [MIT License](LICENSE), matching the original Sveltia CMS license. All original work copyright (c) [Kohei Yoshino](https://github.com/kyoshino) and Sveltia CMS contributors. All enhancements copyright (c) contributors to this fork.
